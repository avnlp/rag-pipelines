...
from rag_pipelines.baml_client.types import PubMedAnswer
...
class TestPubMedQARAG:
    ...
    async def test_answer_generation_node(self):
        ...
        This test ensures that the node correctly calls the BAML `GeneratePubMedAnswer`
        function and processes the structured output.
        """
        # We mock the expected output type `PubMedAnswer` for the BAML function
        mock_baml_response = PubMedAnswer(
            answer="This is the mocked BAML answer.",
            chain_of_thought="mocked reasoning",
        )

        with patch(
            "rag_pipelines.pubmedqa.pubmedqa_rag.b.GeneratePubMedAnswer",
            ...
            # the `answer` from our mock BAML response.
    ...
    @pytest.mark.asyncio
    @patch("rag_pipelines.pubmedqa.pubmedqa_rag.b.GeneratePubMedAnswer")
    ...
    @patch("rag_pipelines.pubmedqa.pubmedqa_rag.MetadataEnricher")
    ...
    @patch(
        ...
        return_value={
            ...
            "baml": {"metadata_function": "ExtractPubmedQAMetadata"},
            "metadata_schema": {"properties": {"test_field": {"type": "string"}}},
        },
    )
    async def test_main_with_mocked_dependencies(
        ...
        mock_generate_answer.return_value = PubMedAnswer(
