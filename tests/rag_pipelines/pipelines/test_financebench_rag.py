"""Test the financebench RAG module."""
...
from rag_pipelines.financebench.financebench_rag import (
...
class TestFinancebenchRAG:
    """Test the financebench RAG module components."""
    ...
    def test_evaluation_node(self):
        """Test the EvaluationNode functionality."""
        # Create mock metric
        mock_metric = Mock(spec=BaseMetric)
        mock_metric.__name__ = "TestMetric"

        # Create a mock evaluation result
        mock_evaluation_result = Mock()
        mock_evaluation_result.test_results = [Mock()]
        mock_evaluation_result.confident_link = "http://test-link.com"

        # Patch the evaluate function to return our mock result
        with patch(
            "rag_pipelines.financebench.financebench_rag.evaluate",
            return_value=mock_evaluation_result,
        ):
            node = EvaluationNode([mock_metric])
            ...
            # Verify that evaluate was called with the correct parameters
            assert len(result["evaluation_scores"]["test_results"]) == 1
            assert (
                result["evaluation_scores"]["confident_link"] == "http://test-link.com"
            )

    @patch("rag_pipelines.financebench.financebench_rag.evaluate")
    @patch("rag_pipelines.financebench.financebench_rag.load_dataset")
    @patch("rag_pipelines.financebench.financebench_rag.ChatGroq")
    @patch("rag_pipelines.financebench.financebench_rag.HuggingFaceEmbeddings")
    @patch("rag_pipelines.financebench.financebench_rag.Milvus")
    @patch("rag_pipelines.financebench.financebench_rag.ContextualReranker")
    @patch("rag_pipelines.financebench.financebench_rag.MetadataExtractor")
    @patch("rag_pipelines.financebench.financebench_rag.ContextualRecallMetric")
    @patch("rag_pipelines.financebench.financebench_rag.ContextualPrecisionMetric")
    @patch("rag_pipelines.financebench.financebench_rag.ContextualRelevancyMetric")
    @patch("rag_pipelines.financebench.financebench_rag.AnswerRelevancyMetric")
    @patch("rag_pipelines.financebench.financebench_rag.FaithfulnessMetric")
    @patch("rag_pipelines.financebench.financebench_rag.load_dotenv")
    ...
    def test_main_with_mocked_dependencies(
        ...
        mock_dataset.return_value = Dataset.from_dict(
            {
                "prompt": ["test question 1", "test question 2"],
                "ideal_completion": ["test answer 1", "test answer 2"],
            }
        )
        ...
        # Set __name__ for DeepEval compatibility
        mock_contextual_recall_instance.__name__ = "ContextualRecallMetric"
        mock_contextual_precision_instance.__name__ = "ContextualPrecisionMetric"
        mock_contextual_relevancy_instance.__name__ = "ContextualRelevancyMetric"
        mock_answer_relevancy_instance.__name__ = "AnswerRelevancyMetric"
        mock_faithfulness_instance.__name__ = "FaithfulnessMetric"
