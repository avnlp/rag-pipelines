"""Test the healthbench RAG module."""
...
from rag_pipelines.baml_client.types import HealthBenchAnswer
from rag_pipelines.healthbench.healthbench_rag import (
...
class TestHealthBenchRAG:
    """Test the HealthBench RAG module components."""
    ...
    async def test_answer_generation_node(self):
        ...
        `GenerateHealthBenchAnswer` function and processes the structured output.
        """
        # We mock the expected output type `HealthBenchAnswer` for the BAML function
        mock_baml_response = HealthBenchAnswer(
            summary="This is the mocked BAML answer.",
            chain_of_thought="mocked reasoning",
        )

        with patch(
            "rag_pipelines.healthbench.healthbench_rag.b.GenerateHealthBenchAnswer",
            ...
            # the `summary` from our mock BAML response.
    ...
    @pytest.mark.asyncio
    @patch("rag_pipelines.healthbench.healthbench_rag.b.GenerateHealthBenchAnswer")
    @patch("rag_pipelines.healthbench.healthbench_rag.evaluate")
    @patch("rag_pipelines.healthbench.healthbench_rag.load_dataset")
    @patch("rag_pipelines.healthbench.healthbench_rag.HuggingFaceEmbeddings")
    @patch("rag_pipelines.healthbench.healthbench_rag.Milvus")
    @patch("rag_pipelines.healthbench.healthbench_rag.ContextualReranker")
    @patch("rag_pipelines.healthbench.healthbench_rag.MetadataEnricher")
    @patch("rag_pipelines.healthbench.healthbench_rag.ContextualRecallMetric")
    @patch("rag_pipelines.healthbench.healthbench_rag.ContextualPrecisionMetric")
    @patch("rag_pipelines.healthbench.healthbench_rag.ContextualRelevancyMetric")
    @patch("rag_pipelines.healthbench.healthbench_rag.AnswerRelevancyMetric")
    @patch("rag_pipelines.healthbench.healthbench_rag.FaithfulnessMetric")
    @patch("rag_pipelines.healthbench.healthbench_rag.GEval")
    @patch("rag_pipelines.healthbench.healthbench_rag.load_dotenv")
    ...
    async def test_main_with_mocked_dependencies(
        ...
        mock_generate_answer,  # 'b.GenerateHealthBenchAnswer'
    ):
        """Test the main function with mocked dependencies to ensure it executes."""
        mock_generate_answer.return_value = HealthBenchAnswer(
        ...
        mock_dataset.return_value = Dataset.from_dict(
            {
                ...
                "ideal_completion": [{"ideal_completion": "test answer"}],
            }
        )
        ...
        mock_g_eval.return_value = mock_g_eval_instance
