"""Evaluation of RAG pipeline on Earnings Calls dataset."""
...
class AnswerGenerationNode:
    ...
    This node is responsible for calling the BAML-defined `GenerateAnswer`
    function, which encapsulates the LLM prompt, client, and output parsing.
    """

    async def __call__(self, state: RAGState) -> RAGState:
        """Generates an answer by invoking the BAML `GenerateAnswer` function.
        ...
        baml_response = await b.GenerateAnswer(context=context_text, question=question)

        # The response from BAML is a structured object (`Answer` class),
        # The final answer is stored in the summary field
        response = baml_response.summary
...
async def main() -> None:
    """Run Earnings Calls RAG pipeline evaluation.
    ...
    the pipeline on the Earnings Calls dataset.
    ...
    with open("earnings_calls_rag_config.yml", "r") as f:
