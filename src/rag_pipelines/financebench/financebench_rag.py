"""Evaluation of RAG pipeline on FinanceBench dataset."""
...
class AnswerGenerationNode:
    ...
    This node is responsible for calling the BAML-defined `GenerateFinanceBenchAnswer`
    function, which encapsulates the LLM prompt, client, and output parsing.
    """

    async def __call__(self, state: RAGState) -> RAGState:
        """Generate answer by invoking the BAML `GenerateFinanceBenchAnswer` function.
        ...
        baml_response = await b.GenerateFinanceBenchAnswer(
            context=context_text, question=question
        )

        # The response from BAML is a structured object (`FinanceBenchAnswer` class),
...
async def main() -> None:
    """Run FinanceBench RAG pipeline evaluation.
    ...
    the pipeline on the FinanceBench dataset.
    ...
    with open("financebench_rag_config.yml", "r") as f:
