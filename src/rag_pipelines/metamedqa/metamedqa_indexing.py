"""Process and Index the MetaMedQA dataset."""
...
async def main() -> None:
    ...
    # Load config
    with open("metamedqa_indexing_config.yml", "r") as f:
    ...
    # Build document list from USMLE Textbooks corpus
    contexts = []
    for context_dict in dataset["context"]:
        contexts.extend(context_dict["contexts"])
    documents = [Document(page_content=doc) for doc in contexts]
